...
/**
 * @brief Blocking reservation acquisition, guarded against the gpu-tier-pin
 * livelock.
 *
 * cucascade::memory_space::make_reservation() blocks on a notification
 ...
 * executor, so a demand larger than (reservation limit - unevictable pinned
 * bytes) can NEVER be granted in full — the wait livelocks forever.
 *
 * Happy path is byte-identical to make_reservation(): the same non-blocking
 * first attempt, nothing else. Only when that attempt fails do we:
