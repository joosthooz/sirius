...
inline size_t dir_bytes(size_t n_chunks) { return sizeof(FrameHeader) + n_chunks * 2 * sizeof(std::uint64_t); }
  ...
  cudf::column_view col, rmm::cuda_stream_view stream, rmm::device_async_resource_ref mr)
{
  ...
  // both sized for the largest (chunk_uncomp) chunk.
  rmm::device_buffer src(chunk_uncomp, stream, mr);
  rmm::device_buffer tmp(comp.configure_compression(chunk_uncomp).temp_buffer_size, stream, mr);
  stream.synchronize();
  ...
  for (size_t c = 0; c < n_chunks; ++c) {
    ...
    if (padded > len) {
      check_cuda(cudaMemsetAsync(static_cast<std::uint8_t*>(src.data()) + len, pad_sym, padded - len,
                                 stream.value()),
                 "memset pad");
    }
    check_cuda(cudaMemcpyAsync(src.data(), col.head<std::uint8_t>() + off, len,
  ...
  check_cuda(cudaMemcpyAsync(payload->data(), host_dir.data(), host_dir.size(),
                             cudaMemcpyHostToDevice, stream.value()),
             "write frame dir");
  for (size_t c = 0; c < n_chunks; ++c) {
    check_cuda(cudaMemcpyAsync(static_cast<std::uint8_t*>(payload->data()) + comp_off[c],
                               chunk_dst[c]->data(), comp_size[c], cudaMemcpyDeviceToDevice,
                               stream.value()),
               "pack chunk");
  }
  stream.synchronize();

  return std::make_unique<fsst_compressed_representation>(dt, n_rows, std::move(payload),
                                                          payload_bytes, bytes);
}
  ...
  rmm::cuda_stream_view stream, rmm::device_async_resource_ref mr) const
{
  ...
  check_cuda(cudaMemcpy(dir.data(), base + sizeof(fh), dir.size() * sizeof(std::uint64_t),
  ...
  for (size_t c = 0; c < n_chunks; ++c) {
    ...
    size_t out_size = 0;
    auto const st   = comp.decompress(base + dir[2 * c],
                                    static_cast<std::uint8_t*>(out.data()) + off, dcfg, &out_size);
    if (st != gtsst::gtsstSuccess) {
