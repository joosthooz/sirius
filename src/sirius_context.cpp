...
void SiriusContext::QueryBegin(ClientContext& context)
{
  ...
  if (config_.is_scan_caching_enabled()) {
    task_creator_->get_scan_executor().cache_scan_results_for_query(query);
  }
...
void SiriusContext::initialize(const sirius::sirius_config& config)
{
  ...
  task_creator_ =
    std::make_unique<sirius::creator::task_creator>(config_.get_gpu_pipeline_executor_config(),
                                                    config_.get_duckdb_scan_executor_config(),
                                                    *memory_manager_,
                                                    &config_.get_hw_topology());

  // Configure scan caching based on config
  task_creator_->get_scan_executor().set_scan_caching_enabled(config_.is_scan_caching_enabled());
