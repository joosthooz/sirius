...
{
  duckdb::initialize_test_buffer_manager();
...
void convert_batch_to_host(std::shared_ptr<data_batch> const& batch)
{
  ...
  auto* host_space = reservation
                       ? manager->get_memory_space(reservation->tier(), reservation->device_id())
                       : nullptr;
  if (!host_space) { throw std::runtime_error("Failed to reserve host memory for test"); }
...
TEST_CASE("GPUPhysicalMaterializedCollector sink with host input",
          "[operator][result_collector][gpu_result_collector]")
{
  ...
  {
    REQUIRE(batch->try_to_lock_for_processing(gpu_space->get_id()).success);
    cucascade::data_batch_processing_handle handle(batch.get());
  ...
  auto plan       = duckdb::make_uniq<duckdb::GPUPhysicalDummyScan>(types, 0);
  auto gpu_prepared =
    duckdb::make_shared_ptr<duckdb::GPUPreparedStatementData>(prepared, std::move(plan));
  duckdb::GPUPhysicalMaterializedCollector collector(*gpu_prepared);

  auto global_state = collector.GetGlobalSinkState(client_ctx);
  auto result       = collector.sink(batch, *global_state);
  REQUIRE(result == duckdb::SinkResultType::FINISHED);
  REQUIRE(collector.result_collection);
  REQUIRE(collector.result_collection->size() == num_rows);

  size_t row_base = 0;
  for (;;) {
    auto chunk = collector.result_collection->GetNext();
    if (!chunk) { break; }
...
TEST_CASE("GPUPhysicalMaterializedCollector sink converts GPU input",
          "[operator][result_collector][gpu_result_collector]")
{
  duckdb::DuckDB db(nullptr);
  duckdb::Connection con(db);
