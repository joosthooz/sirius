...
TEST_CASE("multiple_blocks_allocation_accessor - basic operations", "[memory][accessor]")
{
...
TEST_CASE("multiple_blocks_allocation_accessor - int64_t type", "[memory][accessor]")
{
...
//===----------------------------------------------------------------------===//
// Test: multiple_blocks_allocation_accessor - int32_t type
//===----------------------------------------------------------------------===//

TEST_CASE("multiple_blocks_allocation_accessor - int32_t type", "[memory][accessor]")
{
  using accessor_type = multiple_blocks_allocation_accessor<int32_t>;

  SECTION("get uses element offsets across blocks")
  {
    accessor_type accessor;
    ...
    for (int32_t i = 0; i < 10; ++i) {
      accessor.set_current(100 + i, allocation);
      accessor.advance();
    }

    accessor.set_cursor(1008);
    auto const saved_block  = accessor.block_index;
    auto const saved_offset = accessor.offset_in_block;

    REQUIRE(accessor.get(0, allocation) == 100);
    REQUIRE(accessor.get(5, allocation) == 105);
    REQUIRE(accessor.get(6, allocation) == 106);
    REQUIRE(accessor.get(9, allocation) == 109);
    REQUIRE(accessor.block_index == saved_block);
    REQUIRE(accessor.offset_in_block == saved_offset);
  }
}
...
TEST_CASE("multiple_blocks_allocation_accessor - edge cases", "[memory][accessor]")
{
...
TEST_CASE("multiple_blocks_allocation_accessor - large operations", "[memory][accessor]")
{
...
TEST_CASE("multiple_blocks_allocation_accessor - memcpy_to", "[memory][accessor]")
{
  numa_region_pinned_host_memory_resource upstream_mr(0);
  auto mr = std::make_unique<fixed_size_host_memory_resource>(
    0, upstream_mr, 1024 * 16 * 1, 1024 * 16 * 1, 1024, 16, 1);
...
TEST_CASE("multiple_blocks_allocation_accessor - reset_cursor", "[memory][accessor]")
{
  numa_region_pinned_host_memory_resource upstream_mr(0);
  auto mr = std::make_unique<fixed_size_host_memory_resource>(
    0, upstream_mr, 1024 * 16 * 1, 1024 * 16 * 1, 1024, 16, 1);
